...
// Updated computed properties for logo and site name
const siteLogo = computed(() => {
  if (logoError.value || !settingsStore.getSiteLogo) {
    return defaultLogo
  }
  return settingsStore.getSiteLogo
})

const siteName = computed(() => settingsStore.getSiteName || 'PawnPal')
...
onMounted(async () => {
  ...
  try {
    ...
    // Will fallback to default logo and name
  }
})
